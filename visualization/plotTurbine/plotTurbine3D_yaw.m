function h = plotTurbine3D_yaw(azimuthAngle,yawAngle,xTower,yTower,zHub)
if nargin <= 0
...
r_rotor = 60; % Rotor dimensions
r_tower = 5; % Tower dimensions
...
plot3(xy_circ_Yaw(1,:)+xTower,xy_circ_Yaw(2,:)+yTower,z_circ+zHub,'k--');
grid on
axis equal tight
xlabel('x (m)');
ylabel('y (m)');
...
for j = 1:length(bladeAngles)
    ...
    plot3(xTower+xy_blade_Yaw(1,:),yTower+xy_blade_Yaw(2,:),z_blade+zHub,'k.','markerSize',10,'Tag','blade'); 
    hold on
end